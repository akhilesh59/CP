...
vector<T> slicing(vector<T> const& v,ll X, ll Y) { 
    auto first = v.begin() + X; auto last = v.begin() + Y + 1; 
    vector<T> vector(first, last); return vector; 
}
...
vector<ll> prFactUsingSieve(ll n) {
    vector<ll> arr(n+1);
    vector<ll> ans;
...
void sol()
{
    ...
    for(ll i=0; i<n; i++) {
        for(ll j=i; j<n; j++) {
            vl sub = slicing(v,i,j);
            ans += sub.size();
            for(auto k : sub) ans += (k == 0);
        }
    }
    cout<<ans<<'\n';
}
