...
#define         mine(a)                 (min_element((a).begin(), (a).end()) - v.begin())
#define         maxe(a)                 (max_element((a).begin(), (a).end()) - v.begin())
...
void sol()
{
    ...
    if(n > 3) ans = maxi*n;
    else if(n == 2) ans = max(sum, 2*abs(v[0] - v[1]));
    else if(n == 3) {
        ans = max({3LL*v[0], 3LL*v[2], sum, 3LL*abs(v[0]-v[1]), 3LL*abs(v[1]-v[2])});
    }
    ...
    // ll diff = abs(v[maxid] - v[minid]);             
    // // deb(diff);

    // // if(diff < avg) return void(cout<<sum<<'\n');

    // vl pref(n); pref[0] = v[0];
    // forl(1,n,1) pref[i] = v[i] + pref[i-1];

    // ll len = abs(maxid - minid) + 1;                
    // // deb(len);
    // ll tmp = diff*len;
    // ll rem = (maxid > minid) ? (pref[maxid] - (minid > 0 ? pref[minid - 1] : 0)) : (pref[minid] - (maxid > 0 ? pref[maxid - 1] : 0)) ;
