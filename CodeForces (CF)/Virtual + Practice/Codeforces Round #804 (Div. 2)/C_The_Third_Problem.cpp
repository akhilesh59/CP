...
void sol()
{
    ...
    if(v[0] == 1) return void(cout<<"1\n");
    ll idx = -1;
    
    // vl mex(n);
    // for(ll i=0; i<n; i++) mex[i] = i;
    ...
    for(ll i=0; i<idx; i++) left.pb(v[i]);
    sort(complete(left));
    for(ll i=idx+1; i<n; i++) right.pb(v[i]);
    sort(complete(right));
    ...
    ll ans = facto(((ll)left.size()-1));
    ll x = facto(((ll)right.size()-1));

    // deb2(ans,x);
    
    ll cnt = 0;
    if(((ll)right.size() && (ll)left.size()) && right[0] > left[0]) {
        cnt = (ll)left.size() - 2;
        cnt += (ll)right.size()-1;
    }
