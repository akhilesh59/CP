...
void sol()
{
    ...
    ll idx = -1;
    Fln {
        if((s[i]-'0') + (s[i+1]-'0') > 9) idx = i;
    }
    idx = (idx == -1) ? 0 : idx;

    string ans="";
    // if(idx) ans = s.substr(0,idx) + to_string((s[idx]-'0') + (s[idx+1]-'0')) + s.substr(idx+2,n-idx-3);
    // else ans = to_string((s[0]-'0') + (s[1]-'0')) + s.substr(2,n-2);

    for(ll i=0; i<idx; i++) ans += s[i];
    ans += to_string((s[idx]-'0') + (s[idx+1]-'0'));
    for(ll i=idx+2; i<n; i++) ans += s[i];
    cout<<ans<<'\n';
