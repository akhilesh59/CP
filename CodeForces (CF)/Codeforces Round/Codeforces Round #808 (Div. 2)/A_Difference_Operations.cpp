...
void sol()
{
    ...
    if(v[0] == 1) return void(cout<<"YES\n");

    for(ll i=1; i<n; ++i) {
        if(v[i]%v[i-1] == 1 && v[i]<v[i-1]) return void(cout<<"YES\n");
        if(v[i]%v[i-1] == 0) continue;
        else return void(cout<<"NO\n");
    }

    // if(n == 2 && (v[1]%v[0] == 0 || v[0] == 1)) return void(cout<<"YES\n");
    // else if(v[1] == 1 || (v[1]%v[0] == 1 && v[0]<v[1]) || v[1]-v[0] == 1) return void(cout<<"YES\n");
