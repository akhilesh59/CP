...
bool dfs(vl adj[], vl& vis, vl& mark, ll node, ll m) {
    if(vis[node]) return (mark[node] == m);
    vis[node] = 1;
    mark[node] = m;

    for(auto it : adj[node]) {
        if(dfs(adj, vis, mark, it, (3LL-m)) == false)  return false;
    }
...
void sol()
{
    ...
    for(auto it : mp) if(it.S > 2) return void(cout<<"NO\n");

    for(ll i=1; i<=n; i++) {
        if(!vis[i] && !dfs(adj, vis, mark, i, 1)) return void(cout<<"NO\n");
    }
