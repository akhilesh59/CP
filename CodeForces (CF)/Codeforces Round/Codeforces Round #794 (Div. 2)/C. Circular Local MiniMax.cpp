...
void sol()
{
    ...
    //     if(ans[k] != -1) continue;
    //     ans[k] = v[i++];
    //     if(ans[k+1] == -1) ans[k+1] = v[j--];
    // }

    ll i=0;
	
	for(ll j=0; j<n; j+=2) ans[j] = v[i++];
	for(ll j=1; j<n; j+=2) ans[j] = v[i++];

    bool f = 1, st = 1;

    // for(ll k = 1; k<n-1; k++) {
    //     if(ans[k] > ans[k-1] && ans[k] > ans[k+1]) continue;
    //     else if(ans[k] < ans[k-1] && ans[k] < ans[k+1]) continue;
    //     else {
    //         f = 0;
    //         break;
    //     }
    ...
    // if(ans[0] > ans[n-1] && ans[0] > ans[1]) st = 1;
    // else if(ans[0] < ans[n-1] && ans[0] < ans[1]) st = 1;

    if(ans[0] >= ans[1] || ans[0] >= ans[n-1]) st = 1;
	
	for(ll i=1; i<n;i+=2) if(ans[i] <= ans[i-1] || ans[i] <= ans[(i+1)%n]) f = 0;
	for(ll i=0; i<n; i+=2) if(ans[i] >= ans[(i-1+n)%n] || ans[i] >= ans[(i+1)%n]) f = 0;
