...
void factorial_Subsets_Sum() {
    ll possible = binpow2(2,16);
    ll n = fac.size();

    for(ll i=0; i<possible; i++) {
        ll subset_Sum = 0, element_count = 0;
        for(ll j=0; j<n; j++) {
            if((1<<j) & i) {
                subset_Sum += fac[j];
                element_count++;
        ...
        vp.pb({subset_Sum,element_count});
    }
}
...
void sol()
{
    ...
    for(ll i=0; i<vp.size(); i++) {
        ...
        if(n >= vp[i].first) {
            ll rem = n - vp[i].first;
            bit_count += vp[i].second;
            bit_count += __builtin_popcountll(rem);
