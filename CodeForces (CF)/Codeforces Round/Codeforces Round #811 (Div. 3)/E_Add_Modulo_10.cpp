...
void sol()
{
    ...
    for(auto it : v) {
        if(it%10 == 0) st1.insert(it);
        else if(it%10 == 5) st1.insert(it+5LL);
        else {
            while(it % 10 != 2) it += (it%10);
            st2.insert(it);
        }
    }
    bool chk = 1;
    if(st1.size() > 1 || (st1.size()>0 && st2.size()>0)) chk = 0;
    set<ll> st3;

    for(auto it : st2) st3.insert(it%20);
