...
void sol()
{
    ...
    sort_asc(inc);
    if(v == inc) return void(cout<<"0\n");
    sort_dec(dec);
    if(v == dec) return void(cout<<dec[0]<<'\n');

    ll maxi = LLONG_MIN;

    for(ll i=1; i<n; ++i) {
        if(v[i] < v[i-1]) {
            maxi  = max(maxi, ((v[i]+v[i-1])/2LL));
        }
    }

    // deb(maxi);

    if(chk(v, maxi)) return void(cout<<maxi<<'\n');
    else if(chk(v, maxi+1)) return void(cout<<(maxi+1LL)<<'\n');
