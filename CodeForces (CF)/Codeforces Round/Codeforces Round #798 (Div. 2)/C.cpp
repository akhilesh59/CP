...
pair<ll,ll> count(vl adj[], ll root, vl& vis) {
    ll cnt = 0, l = 0;
    queue<ll> q;
    q.push(root);
    ...
    return cnt > 1 ? mkpr(--cnt, l) : mkpr(0LL,l);
}
...
void sol()
{
    ...
    while(!q.empty() && op) {
        ...
        for(ll i=0; i<sz; i++) {
            ...
            cnt++;
            op--;
            ll maxi = INT_MIN, maxsz = 0, minh = INT_MAX;

            for(auto it : adj[node]) {
                pair<ll,ll> tmp = count(adj, it, vis);
                // if(tmp > maxsz) {
                //     maxsz = tmp;
                //     maxi = it;
                // }
