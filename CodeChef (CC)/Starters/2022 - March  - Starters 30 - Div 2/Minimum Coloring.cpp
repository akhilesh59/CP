...
bool isSafe(vector<vector<ll>>& mat, ll x, ll y, ll num) {
    ll n = mat.size();
    ll m = mat[0].size();

    if(y+1 < m && mat[x][y+1] == num) return false;
    if(y-1 > 0 && mat[x][y-1] == num) return false;
    if(x+1 < n && mat[x+1][y] == num) return false;
    if(x-1 > 0 && mat[x-1][y] == num) return false;
